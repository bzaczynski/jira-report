...
def main(args: argparse.Namespace) -> None:
    ...
    if os.path.exists(filename) and not args.force_overwrite:
        ...
        if len(issues) > 0:
            ...
            xls_export(issues, title, filename)
        else:
...
def xls_export(issues: List[jira.Issue],
    ...
    for row, issue in enumerate(issues, 1):
        ...
        write(sheet, row, 3, make_datetime(issue.fields.created), date_format)
        write(sheet, row, 4, issue.fields.creator.displayName, middle)
        write(sheet, row, 5, issue.fields.project.name, middle)
        write(sheet, row, 6, issue.fields.status.name, middle)
        write(sheet, row, 7, issue.fields.summary, middle)
